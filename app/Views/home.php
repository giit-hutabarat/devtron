...
html, body { height: 100%; margin: 0; padding: 0; overflow: hidden; font-family: 'Roboto', sans-serif; }/* BACKGROUND */.bg-container {background-image: url('');background-size: cover; background-position: center;width: 100vw; height: 100vh; position: relative;}.bg-overlay {background: rgba(15, 23, 42, 0.9);width: 100%; height: 100%;display: flex; flex-direction: column; align-items: center; justify-content: center;padding: 20px; overflow-y: auto;}/* CARD MENU */.card-menu {border-radius: 24px !important; border: 1px solid rgba(255,255,255,0.05);backdrop-filter: blur(10px); transition: all 0.4s; cursor: pointer;}.card-menu:hover { transform: translateY(-10px); border-color: rgba(255,255,255,0.2); box-shadow: 0 20px 40px rgba(0,0,0,0.6) !important; }.icon-circle {width: 90px; height: 90px; border-radius: 50%; background: rgba(255,255,255,0.05);display: flex; align-items: center; justify-content: center; margin: 0 auto 20px auto; transition: 0.3s;}.card-menu:hover .icon-circle { transform: scale(1.1) rotate(5deg); background: rgba(255,255,255,0.1); }.menu-title { font-size: 1.4rem; font-weight: 900; letter-spacing: 1px; margin-bottom: 5px; text-transform: uppercase; }.menu-desc { font-size: 0.85rem; color: #94a3b8; margin-bottom: 25px; min-height: 40px; }.btn-akses { font-weight: 700; border-radius: 50px; padding: 0 30px !important; }/* FOOTER */.fixed-footer { position: absolute; bottom: 20px; left: 0; width: 100%; text-align: center; z-index: 10; }.footer-text {color: #64748b; font-size: 0.85rem; font-weight: 500; letter-spacing: 2px; text-transform: uppercase;border-top: 1px solid rgba(255,255,255,0.1); padding: 15px 30px 0; display: inline-block;}/* PRELOADER */#preloader {position: fixed; top: 0; left: 0; width: 100%; height: 100%;background-color: #0a0a0a; z-index: 99999;display: flex; flex-direction: column; align-items: center; justify-content: center;transition: opacity 0.8s ease-out;}.logo-pulse { width: 120px; animation: pulseLogo 1.5s infinite; }.loader-text { color: #666; margin-top: 20px; font-size: 0.8rem; letter-spacing: 3px; text-transform: uppercase; animation: blink 1s infinite; }@keyframes pulseLogo { 0% { transform: scale(1); opacity: 1; } 50% { transform: scale(1.1); opacity: 0.8; } 100% { transform: scale(1); opacity: 1; } }@keyframes blink { 0%, 100% { opacity: 1; } 50% { opacity: 0.5; } }@keyframes shake { 0%, 100% { transform: translateX(0); } 25% { transform: translateX(-5px); } 75% { transform: translateX(5px); } }.v-shake { animation: shake 0.3s; }

        /* ... CSS ANDA (Tetap Sama) ... */
        /* Pastikan CSS Vuetify Anda tidak menimpa font-family */
        .icon-circle .v-icon { font-family: 'Font Awesome 6 Free', sans-serif !important; }
        /* ... */
    </style>
</head>
...
<body>
    ...
    <v-card 
        ...
        color="#450a0a" 
        dark 
        href="<?= site_url('sidang/access'); ?>"  ripple 
        elevation="15" 
    ...
    >
        ...
        <v-btn color="#ef4444" class="btn-akses white--text elevation-5">Masuk Menu <v-icon right small>fas fa-sign-in-alt</v-icon></v-btn>
    </v-card>
</v-col>    
...
<v-dialog v-model="modalAuth" persistent max-width="400px">
    <v-card color="#1e293b" dark class="rounded-xl pa-5 elevation-24">
        <v-card-title class="justify-center text-h5 font-weight-bold text-blue-lighten-3 mb-4">
            {{ targetUrl === 'sidang_otp' ? 'VERIFIKASI AKSES (2FA)' : 'LOGIN ADMIN' }}
        </v-card-title>
        <v-card-text class="text-center pb-0">
            ...
            <v-form v-if="targetUrl === 'dashboard'" ref="formLogin" v-model="valid" @submit.prevent="loginProcess">
                ...
                <v-text-field v-model="loginUsername" :rules="[rules.required]" label="Username / Email" outlined dense rounded color="light-blue lighten-3" prepend-inner-icon="fas fa-user"></v-text-field>
            ...
            <v-form v-else-if="targetUrl === 'sidang_otp'">
                <p class="red--text text--accent-2 font-weight-bold">Akses Modul Sidang Dialihkan</p>
                <p class="white--text caption">Silakan klik tombol "Masuk Menu" untuk diarahkan ke halaman verifikasi 2FA terpisah.</p>
                <v-icon size="60" color="#ef4444">fas fa-gavel</v-icon>
            </v-form>
