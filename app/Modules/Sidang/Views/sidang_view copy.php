...
<head>
    ...
    <style>
        ...
        /* Responsive */
        .main-container { flex: 1; display: flex; justify-content: center; align-items: flex-start; padding: 10px; overflow-y: auto; } /* Padding lebih kecil di HP */
    .card-custom { 
        background-color: #1e1e1e; border: 1px solid #333; border-radius: 12px; width: 100%; 
        max-width: 100%; /* Ubah dari 1100px agar responsif penuh, atau biarkan 1100px jika memang untuk desktop */
        box-shadow: 0 10px 30px rgba(0,0,0,0.5); 
    }
    .filter-box { background: #252525; border-bottom: 1px solid #333; padding: 15px; } /* Padding lebih kecil */
    .custom-check-group { 
        /* Pastikan group checkbox tidak terlalu lebar */
        display: flex; gap: 15px; align-items: center; 
        height: 100%; background: #2c2c2c; border: 1px solid #444; 
        padding: 6px 15px; border-radius: 6px; 
    }
    
    /* Media Query untuk penyesuaian di layar kecil (Mobile) */
    @media (max-width: 767.98px) {
        ...
        .card-header-custom > div:first-child { margin-bottom: 10px; }
        .btn-sync { 
            width: 100%; 
            text-align: center; 
            padding: 5px 10px;
        }
        /* Penyesuaian layout filter: Button cetak harus berada di bawah */
        .filter-box .row > div { margin-bottom: 10px; }
        .filter-box .row > div:last-child { margin-bottom: 0; } 
        .table-responsive { padding: 0 5px 15px 5px; } /* Padding lebih kecil */
        
        /* Pastikan custom-check-group tidak terpotong */
        .custom-check-group { 
            flex-wrap: wrap; /* Memungkinkan wrap jika tidak muat */
        }
        
        /* Paksa tombol di kolom terakhir untuk stacked di HP */
        .col-md-4.text-end { 
            text-align: left !important; /* Agar tombol tidak mepet kanan di HP */
            display: flex; /* Gunakan flex untuk menumpuk 2 tombol cetak */
            flex-direction: column;
        }
        .btn-success-custom, .btn-cetak {
...
<body>
    ...
    <div class="main-container">
        ...
        <div class="card-header-custom">
                ...
                <a href="<?= $base_url . '/sidang/sync' ?>" class="btn-sync mt-2 mt-md-0" onclick="return confirm('Proses ini akan mengambil data terbaru dari Google Sheet dan menyimpannya ke Database. Lanjutkan?');">
