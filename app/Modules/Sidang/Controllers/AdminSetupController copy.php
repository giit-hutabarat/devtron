...
class AdminSetupController extends BaseController
{
    ...
    public function saveNip()
    {
        ...
        if (empty($input['id'])) {
            $validationRules['nip'] .= 'required|min_length[5]|is_unique[sidang_admins.nip]';
        } else{
            $validationRules['nip'] .= 'required|min_length[5]|is_unique[sidang_admins.nip,id,' . $input['id'] . ']';
        }
